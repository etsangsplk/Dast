...
void TpccTxn::NewOrderRetry() {
  ...
  n_pieces_dispatchable_ =  2 + ol_cnt;
...
void TpccPiece::RegNewOrder() {
  ...
  // R warehouse
  INPUT_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_1,
            TPCC_VAR_W_ID)
  OUTPUT_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_1,
             TPCC_VAR_W_TAX)
...
//                    mb,
//                    ROW_ORDER_SEC);
    dtxn->WriteColumn(r, 3, cmd.input[TPCC_VAR_W_ID], TXN_BYPASS);
    return;
  } END_PIE
...
//    Log_info("TPCC_NEW_ORDER, piece: %d", TPCC_NEW_ORDER_RI(I));
  ...
  BEGIN_LOOP_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_RS(0), 1000, DF_NO)
    ...
    w_id = cmd.input[TPCC_VAR_S_W_ID(I)].get_i32();
    }
    else{
    mb[1] = cmd.input[TPCC_VAR_W_ID].get_blob();
    w_id = cmd.input[TPCC_VAR_W_ID].get_i32();
    }
