...
bool TpccTxn::CheckReady() {
  ...
  for (auto &pair : status_) {
    ...
    if (all_found && status == WAITING) {
      Log_info("Changing piece %d to dispatchable", pi);
      status = DISPATCHABLE;
      TxnWorkspace& ws = GetWorkspace(pi);
      ws.keys_ = var_set;
...
void TpccTxn::PrepareAllCmds() {
  Log_info("%s called for txn id %lu", __PRETTY_FUNCTION__, this->id_);
  for (auto &pair: status_){
    const innid_t& pi = pair.first;
    int32_t& status = pair.second;

     if (status == WAITING){
       Log_info("Preparing piece %d, its input is not ready", pi);
//       status = DISPATCHABLE;
