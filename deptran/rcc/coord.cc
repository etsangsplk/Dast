...
//  verify(info.partition_.find(cmd.partition_id_) != info.partition_.end());
  ...
  if (txn().HasMoreSubCmdReadyNotOut()) {
    Log_info("command has more sub-cmd, cmd_id: %lu,"
                  " n_started_: %d, n_pieces: %d, type = %d",
              txn().id_, txn().n_pieces_dispatched_, txn().GetNPieceAll(), txn().type());
    Dispatch();
  } else if (AllDispatchAcked()) {
    Log_info("receive all start acks, txn_id: %lu; START PREPARE, type = %u", cmd_->id_, txn().type());
