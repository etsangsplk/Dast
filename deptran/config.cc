...
void Config::LoadYML(std::string &filename) {
  ...
  if (config["chronosparams"]){
    LoadChronosParams(config["chronosparams"]);
  }
...
void Config::LoadAuxiliaryRaft(YAML::Node raft){
  ...
  for (auto it = raft.begin(); it != raft.end(); it++){
    ...
    Log_info("raft server at proc %s, port %d", proc_name.c_str(), port);
...
if (bench_str == "tpcc"){
  ...
  if (config["tpcc_new_order_remote_rate"]){
    tpcc_new_order_remote_rate_ = config["tpcc_new_order_remote_rate"] .as<int>();
  }else{
    tpcc_new_order_remote_rate_ = 1;
  }
  Log_info ("TPCC new_order remote rate = %d%", tpcc_new_order_remote_rate_);
}
