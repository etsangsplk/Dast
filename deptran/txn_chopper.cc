...
//  m << (ws.keys_);
  ...
  DEP_LOG("ws.pieces_input_ready %d", ws.piece_input_ready_);
  for (int32_t k : ws.keys_) {
    if (ws.piece_input_ready_ == 0){
      if (ws.input_var_ready_.at(k) == 1){
        DEP_LOG("key = %d, ws input var ready size = %d, value ready", k, ws.input_var_ready_.size());
        auto it = (*ws.values_).find(k);
        verify(it != (*ws.values_).end());
        m << k << it->second;
      } else{
        //xs: put an EMPTY_VALUE here
        DEP_LOG("key = %d, ws input var ready size = %d, value not ready", k, ws.input_var_ready_.size());
...
//      status = INIT;
      if (status == OUTPUT_READY){
        //Not needed;
        Log_info("piece %d's status is output ready", pi);
        n_not_used_pi++;
        continue;
      }
      ...
      if (status == DISPATCHABLE) {
        ...
        for (auto &k: cmd->input.keys_){
          if (cmd->input.values_->count(k) != 0){
            DEP_LOG("input variable %d ready", k);
            cmd->input.input_var_ready_[k] = 1;
          }else{
            cmd->input.input_var_ready_[k] = 0;
            waiting_var_par_map[k].push_back(cmd->partition_id_);
            DEP_LOG("input variable %d not ready, partition is waiting %d", k, cmd->partition_id_);
          }
        }
  ...
  for (auto &pair: cmds) {
    for (auto& c: pair.second){
      ...
      DEP_LOG("cmd to partition %u, piece id = %u, input ready = %d, input_var_ready.size = %d", pair.first, c->inn_id(), c->input.piece_input_ready_, c->input.input_var_ready_.size());
    }
  }
