...
TxSlog::RecursiveExecuteReadyPieces(parid_t my_par_id) {
...
void TxSlog::CheckSendOutputToClient() {
  ...
  if (this->slog_phase_ < SLOG_DIST_EXECUTED){
    Log_info("Execution not finished");
    return;
  }
  ...
  for (auto& pair : par_output_received_){
    if (pair.second == false){
      Log_info("output for partition %hu not received", pair.first);
      all_output_received = false;
      break;
    }else{
      Log_info("output for partition %hu received", pair.first);
    }
  }
