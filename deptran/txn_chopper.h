...
#define EMPTY_VALUE 0xFFFFEEFF //xs: a magic value
...
class TxnWorkspace {
 public:
  ...
  std::map<int32_t, int32_t> input_var_ready_{}; //Better to also save the dependencies.
  int32_t piece_input_ready_;
  TxnWorkspace();
  ~TxnWorkspace();
...
class SimpleCommand: public ContainerCommand {
 public:
  ...
  bool local_executed_ = false;

  //xs: for piece rw deps
...
class TxnCommand: public ContainerCommand {
 ...
 public:
  ...
  virtual map<parid_t, vector<SimpleCommand*>> GetAllCmds(int32_t max=0);
  ...
  //XS: prepare cmds, even if their input is not ready.
  virtual void PrepareAllCmds(){
